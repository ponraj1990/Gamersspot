...
function App() {
  ...
  // Intelligent merge function to combine local and remote state
  const mergeStations = (localStations, remoteStations) => {
    return localStations.map(localStation => {
      const remoteStation = remoteStations.find(rs => rs.id === localStation.id)
      
      if (!remoteStation) {
        // Remote doesn't have this station, keep local
        return localStation
      }
      
      // Priority 1: If remote marked as done, always accept it (stop local timer if running)
      if (remoteStation.isDone && !localStation.isDone) {
        // Remote marked as done on another device, accept it and stop local timer
        return {
          ...remoteStation,
          isRunning: false, // Stop the timer
          // Use the higher elapsed time (more accurate)
          elapsedTime: Math.max(localStation.elapsedTime || 0, remoteStation.elapsedTime || 0),
          // Accept end time from remote if available
          endTime: remoteStation.endTime || localStation.endTime
        }
      }
      
      // Priority 2: If local timer is running, preserve local elapsed time and running state
      // but accept other changes from remote (like isDone, customerName, etc.)
      if (localStation.isRunning && !remoteStation.isDone) {
        return {
          ...localStation,
          // Accept critical state changes from remote (but not isDone if it would stop the timer)
          customerName: remoteStation.customerName || localStation.customerName,
          extraControllers: remoteStation.extraControllers !== undefined ? remoteStation.extraControllers : localStation.extraControllers,
          snacks: remoteStation.snacks || localStation.snacks,
          // Keep local running state and elapsed time (calculated locally)
          isRunning: localStation.isRunning,
          elapsedTime: localStation.elapsedTime,
          startTime: localStation.startTime || remoteStation.startTime,
          endTime: localStation.endTime || remoteStation.endTime,
        }
      }
      
      // If remote is running and local is not, accept remote (timer started on another device)
      if (remoteStation.isRunning && !localStation.isRunning) {
        return {
          ...remoteStation,
          // Calculate elapsed time based on start_time if available
          elapsedTime: remoteStation.elapsedTime || 0
        }
      }
      
      // Default: merge with remote taking precedence for most fields
      return {
        ...remoteStation,
        // But keep local elapsed time if local is higher (more accurate for running timers)
        elapsedTime: localStation.isRunning 
          ? Math.max(localStation.elapsedTime || 0, remoteStation.elapsedTime || 0)
          : remoteStation.elapsedTime || localStation.elapsedTime || 0
      }
    })
  }
  
  // Sync stations from database
  const syncStationsFromDatabase = async () => {
    ...
    try {
      ...
      const remoteStations = await loadStations()
      
      if (remoteStations && remoteStations.length > 0) {
        setStations(prevStations => {
          if (prevStations.length === 0) {
            // No local stations, use remote
            return remoteStations
          }
          
          // Merge local and remote stations intelligently
          const merged = mergeStations(prevStations, remoteStations)
          
          // Check if anything actually changed
          const hasChanges = merged.some((mergedStation, index) => {
            const prevStation = prevStations[index]
            if (!prevStation) return true
            
            return (
              prevStation.isRunning !== mergedStation.isRunning ||
              prevStation.isDone !== mergedStation.isDone ||
              prevStation.elapsedTime !== mergedStation.elapsedTime ||
              (prevStation.customerName || '') !== (mergedStation.customerName || '') ||
              prevStation.startTime !== mergedStation.startTime ||
              prevStation.endTime !== mergedStation.endTime ||
              prevStation.extraControllers !== mergedStation.extraControllers ||
              JSON.stringify(prevStation.snacks || {}) !== JSON.stringify(mergedStation.snacks || {})
            )
          })
          
          if (hasChanges) {
            console.log('Synced stations from database')
            return merged
          }
          
          return prevStations // No changes, keep local
        })
      }
      
      lastSyncTimeRef.current = Date.now()
    } catch (error) {
      console.error('Error syncing stations from database:', error)
    } finally {
